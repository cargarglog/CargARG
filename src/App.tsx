...
const App: React.FC = () => {
        ...
        (async () => {
            try {
                ...
                if (result?.user) {
                    ...
                    const intendedRole = (sessionStorage.getItem('cargarg:intendedRole') as UserRole) || UserRole.DRIVER;
                    const intendedCompany = sessionStorage.getItem('cargarg:intendedCompany');
                    await firestore.setDoc(userDocRef, {
                        uid: firebaseUser.uid,
                        email: (firebaseUser.email || '').toLowerCase().trim(),
                        role: intendedRole,
                        companyName: intendedRole === UserRole.COMPANY ? (intendedCompany || null) : null,
                        perfilEstado: 'pending_attempt1', verificationStatus: 'pending',
                        plan: 'free',
                        createdAt: firestore.serverTimestamp(),
                    }, { merge: true });
                    sessionStorage.removeItem('cargarg:intendedRole');
                    sessionStorage.removeItem('cargarg:intendedCompany');
                }
            } catch (e) {
    ...
    // Persist intended role/company across Google flows (popup/redirect)
    const INTENDED_ROLE_KEY = 'cargarg:intendedRole';
    const INTENDED_COMPANY_KEY = 'cargarg:intendedCompany';

    const handleGoogleSignIn = async (intendedRole?: UserRole, intendedCompanyName?: string) => {
        ...
        try {
            ...
            if (intendedCompanyName) {
                sessionStorage.setItem(INTENDED_COMPANY_KEY, intendedCompanyName);
            }
            ...
            if (!userDocSnap.exists()) {
                ...
                const storedRole = (sessionStorage.getItem(INTENDED_ROLE_KEY) as UserRole) || UserRole.DRIVER;
                const storedCompany = sessionStorage.getItem(INTENDED_COMPANY_KEY);
                const firestoreData = {
                    uid: firebaseUser.uid,
                    email: (firebaseUser.email || '').toLowerCase().trim(),
                    role: storedRole,
                    companyName: storedRole === UserRole.COMPANY ? (storedCompany || null) : null,
                    perfilEstado: 'pending_attempt1', verificationStatus: 'pending' as const,
                    plan: 'free' as const,
                    createdAt: firestore.serverTimestamp(),
                };
                ...
                // Clear hints after successful creation
