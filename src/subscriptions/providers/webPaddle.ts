...
function productIdFor(plan: Plan): string | null {
  switch (plan) {
    case 'silver':
      return import.meta.env.VITE_PADDLE_PRODUCT_SILVER || null;
    case 'gold':
      return import.meta.env.VITE_PADDLE_PRODUCT_GOLD || null;
    default:
      return null; // free no requiere Paddle
  }
}
...
export function createWebPaddleProvider() {
  const provider = {
            ...
            successCallback: async (data: any) => {
              try {
                ...
                if (r?.data?.ok) {
                  ...
                  resolve({ ok: false, message: 'No se pudo confirmar el pago' });
                }
              } catch (e: any) {
    ...
    async openBillingPortal(): Promise<void> {
      ...
      else throw new Error('No hay URL de actualización de pago disponible');
    },
