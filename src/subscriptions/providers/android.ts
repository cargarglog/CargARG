...
export function createAndroidProvider() {
    ...
    async startPurchase(plan: Plan): Promise<PurchaseResult> {
      ...
      try {
        if (window.AndroidBilling?.startSubscription) {
          // Expected to trigger native flow and later emit a window event with details
          await window.AndroidBilling.startSubscription(sku);
          return { ok: true };
        }
        // Fallback: open subscription management
  ...
  // Listener for native purchase updates (to be called from WebView/Capacitor bridge)
