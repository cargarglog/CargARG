...
type ConfirmedInfo = Record<string, string>;
...
const INCIDENT_KEYWORDS: Array<{ keyword: string; severity: 'medium' | 'high' | 'critical' }> = [
...
export const chatbotFunction = functions.region('us-central1').https.onCall(async (data: ChatbotRequest, context) => {
  if (!context.auth) {
  ...
  const derivedMeta: { transcriptId?: string; notifyLogistics?: boolean; logisticsEventId?: string | null } = {};
  ...
  let confirmedInfo: ConfirmedInfo = {};
  if (tripId) {
...
function sanitizeClientMeta(raw: any): Record<string, any> {
...
async function transcribeAudio(audioBase64: string, languageCode: string): Promise<string> {
...
async function classifyLogisticsIntent(text: string): Promise<boolean> {
  ...
  if (positiveHits === 0 && negativeHits > 0) return false;
        ...
        messages: [
          {
            role: 'system',
            content:
              'Responde SOLO con "si" o "no". Di "si" si el mensaje está relacionado con cargas, transporte o logística. De lo contrario responde "no".',
          },
...
export function maskPII(text: string): MaskResult {
  ...
  const replaceWith = (match: string) => {
    blocked = true;
...
function analyzeIncidents(text: string): { shouldNotify: boolean; severity: 'medium' | 'high' | 'critical'; matches: string[] } {
...
function extractConfirmedInfo(raw: Record<string, any>): ConfirmedInfo {
  const result: ConfirmedInfo = {};

  const walk = (node: any, path: string[], parentConfirmed: boolean) => {
    if (node == null) return;
...
function generateLogisticsReply(message: string, info: ConfirmedInfo): string | null {
  const availableEntries = Object.entries(info);
  if (!availableEntries.length) return null;

  const lower = message.toLowerCase();

  const findByKeys = (substrings: string[]): string | null => {
  ...
  if (/\b(dónde|donde|ubicaci[oó]n|checkpoint|parada|posición|posicion)\b/.test(lower)) {
    const location =
      findByKeys(['location', 'checkpoint', 'posicion', 'ubicacion', 'currentstop']) ||
  ...
  if (/\b(carga|mercader[ií]a|mercancia|producto|material)\b/.test(lower)) {
    const load =
      findByKeys(['load', 'carga', 'shipment', 'mercader', 'contenido']) ||
...
function summarizeForLogistics(content: string, keywords: string[]): string {
...
function formatKey(key: string): string {
  const normalized = key.toLowerCase();
  if (normalized.includes('eta') || normalized.includes('arrival')) return 'ETA confirmada';
  if (normalized.includes('location') || normalized.includes('ubicacion') || normalized.includes('checkpoint')) {
    return 'Ubicación confirmada';
  }
  if (normalized.includes('status') || normalized.includes('estado')) return 'Estado';
  if (normalized.includes('load') || normalized.includes('carga')) return 'Carga';
  if (normalized.includes('document')) return 'Documentación';
  if (normalized.includes('peso') || normalized.includes('weight')) return 'Peso';
